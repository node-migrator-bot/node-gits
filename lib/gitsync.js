...
/**
 * Syncs a GIT repository. If the repository already exists, will pull the recent changes.
...
GitSync.prototype.sync = function(url, branch, dir, callback) {
    ...
    fs.exists(dir, function(exists) {
        ...
        if (exists) {
            ...
            log.info("Checking if " + dir + " contains a valid git repo");
            self.isValid(dir, function(err, valid) {
                ...
                log.warn('Invalid repository in a directory ' + dir);
                // Clone again.
                self.clonebranch(url, branch, dir, callback);
            });
 ...
 * Clones a repository.
 */
GitSync.prototype.clone = function(root, url, branch, dir, callback) {
    // clone the mfkr
    log.info("Cloning branch " + branch + " from " + url + " into " + dir);
    git(root, ['clone', '-b', branch, url, dir], callback);
}

/**
 * Clones a branch. If branch's directory already exists, delete it.
 */
GitSync.prototype.clonebranch = function(url, branch, dir, callback) {
    var self = this;
        ...
        fs.exists(dir, function(exists) {
            ...
            if (exists) {
                rimraf(dir, function(err) {
                    if (err) {
                        log.error('failed to remove branch directory ' + dir + ' err:', err);
                        callback('failed to remove directory ' + dir);
                        return;
                    }
                    self.clone(root, url, branch, dir, callback);
                });
                return;
            }
            self.clone(root, url, branch, dir, callback);
